  ...
  for (const entry of KNOWLEDGE_BASE) {
    ...
    for (const keyword of entry.keywords) {
      if (q.includes(keyword.toLowerCase().replace(/\s+/g, ''))) {
        score += keyword.length // 긴 키워드 매칭에 더 높은 점수
      }
    }
    if (score > bestScore) {
      bestScore = score
...
export default function ChatbotPage() {
  ...
  const [mode, setMode] = useState<'auto' | 'local'>('auto') // auto=Gemini우선→로컬폴백, local=로컬만
        ...
        try {
          ...
          if (e.message === 'QUOTA_EXHAUSTED') {
            ...
            if (localAnswer) {
              ...
              reply = '⚡ Gemini API 일일 할당량이 소진되었습니다.\n\n내장 지식베이스에서도 해당 질문의 답변을 찾지 못했습니다.\n추천 질문을 이용하시거나, 내일 API가 초기화된 후 다시 질문해 주세요.\n\n(할당량은 매일 태평양시간 자정 = 한국시간 오후 4~5시에 초기화됩니다)'
              source = 'local'
            }
    ...
    <div className="space-y-5">
      ...
      {quotaExhaustedRef.current && (
        <div className="card p-4 bg-amber-50 border-amber-200 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-amber-900">Gemini API 할당량 소진</p>
            <p className="text-xs text-amber-700 mt-0.5">
              일일 무료 할당량이 소진되어 <strong>내장 지식베이스 모드</strong>로 자동 전환되었습니다.
              내부회계 관련 주요 질문에 답변 가능합니다. API는 매일 한국시간 오후 4~5시에 초기화됩니다.
            </p>
          </div>
        </div>
      )}
