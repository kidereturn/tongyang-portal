...
type Tab = 'upload-users' | 'upload-rcm' | 'upload-population' | 'users' | 'activities' | 'files' | 'notifications' | 'videos' | 'webtoon'
...
function WebtoonTab() {
  ...
  async function handleDelete(ep: WebtoonEpisode) {
    ...
    await db.from('webtoon_episodes').delete().eq('id', ep.id)
    loadEpisodes()
  }
