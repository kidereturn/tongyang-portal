    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-warm-400 text-sm mt-0.5">증빙 자료를 업로드하고 결재를 상신합니다</p>
        </div>
      </div>
      ...
      <div className="bg-brand-800 border border-slate-800 rounded-xl p-6 space-y-5">
        {/* 활동 선택 */}
        <div>
          <label className="block text-sm font-medium text-warm-300 mb-1.5">
            통제 활동 <span className="text-red-400">*</span>
          </label>
          {activities.length === 0 ? (
            <div className="bg-brand-700 border border-slate-700 rounded-lg px-4 py-3 text-warm-500 text-sm">
            ...
            <select
              ...
              className="w-full bg-brand-700 border border-slate-700 text-white text-sm rounded-lg px-4 py-3 outline-none focus:border-brand-700 focus:ring-2 focus:ring-brand-700/20 transition-all"
            >
        ...
        {/* 비고 */}
        <div>
          <label className="block text-sm font-medium text-warm-300 mb-1.5">비고 / 설명</label>
          <textarea
            value={notes}
        ...
        <div>
          <label className="block text-sm font-medium text-warm-300 mb-1.5">
            증빙 파일 첨부
            <span className="text-warm-500 font-normal ml-1.5">(최대 {MAX_MB}MB · PDF, 이미지, Office, ZIP)</span>
          </label>

          {file ? (
            <div className="flex items-center gap-3 bg-brand-700 border border-brand-700 rounded-lg px-4 py-3">
              <FileText size={20} className="text-brand-400 shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-white text-sm font-medium truncate">{file.name}</p>
                <p className="text-warm-500 text-xs">{(file.size / 1024 / 1024).toFixed(2)} MB</p>
              </div>
              <button
            ...
            <div
              ...
              className={`border-2 border-dashed rounded-xl px-6 py-10 text-center cursor-pointer transition-all ${
                dragging
                  ...
                  : 'border-slate-700 hover:border-warm-500 hover:bg-brand-900/50'
              }`}
            >
              <Upload size={28} className="text-warm-600 mx-auto mb-2" />
              <p className="text-warm-400 text-sm">
                파일을 여기에 드래그하거나 <span className="text-brand-400 font-medium">클릭하여 선택</span>
              </p>
              <p className="text-warm-600 text-xs mt-1">{ACCEPT.replace(/\./g, '').replace(/,/g, ' · ').toUpperCase()}</p>
            </div>
          )}
      ...
      <div className="flex gap-3">
        <button
          ...
          className="flex items-center gap-2 bg-brand-700 hover:bg-slate-700 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-medium px-5 py-2.5 rounded-lg transition-all border border-slate-700"
        >
        ...
        <button
          ...
          disabled={loading || !activityId}
          className="flex items-center gap-2 bg-brand-800 hover:bg-warm-500 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-medium px-5 py-2.5 rounded-lg transition-all shadow-md shadow-brand-900/30"
        >
