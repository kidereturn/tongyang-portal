// 네트워크 50회 반복 테스트 (5초 간격)
// 시나리오: 증빙관리 → 5초 → 스크린샷 → 내 승인함 → 5초 → 스크린샷 → 학습현황 → 5초 → 스크린샷 (1 cycle)
// 150 페이지 로드 전체 스크린샷 저장, 하얀(loading) 화면 감지 시 STUCK 표기
...
const OUT = path.resolve(__dirname, '..', 'screenshots-network-5sec-50')
...
async function main() {
  ...
  try {
    ...
    const stuckEvents = []
    const timings = []
    log(`▶ 시작 — ${TOTAL} iterations × [증빙→승인함→학습현황] each 5s, 스크린샷 ${TOTAL*3}장`)
    ...
    for (let i = 1; i <= TOTAL; i++) {
      ...
      if (iterStuck.length > 0) stuckEvents.push({ iter: i, stuck: iterStuck })
    ...
    report.push('네트워크 안정성 테스트 보고서 — 5초 스크린샷 × 50 iterations')
    report.push('='.repeat(60))
    report.push(`총 소요시간: ${(overallElapsed/1000).toFixed(1)}초 (평균 iter: ${(avgIter/1000).toFixed(1)}초)`)
    report.push(`전체 스크린샷 수: ${TOTAL * 3}장 (증빙/승인함/학습현황 각 ${TOTAL}장)`)
    ...
    report.push('')
    report.push(`스크린샷 저장 위치: screenshots-network-5sec-50/`)
