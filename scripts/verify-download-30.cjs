// 다운로드 회귀 검증 30회 — 신뢰성 있는 selector
const puppeteer = require('puppeteer')
const fs = require('fs')
const path = require('path')

const ROOT = path.join(__dirname, '..')
const OUT = path.join(ROOT, 'screenshots-download-30')
...
async function main() {
  ...
  for (let i = 1; i <= 30; i++) {
    ...
    if (i % 5 === 0) {
      console.log(`[${i}/30] ok=${stats.success} popup=${stats.popup} closeFail=${stats.closeFail} healthFail=${stats.healthFail}`)
    }
